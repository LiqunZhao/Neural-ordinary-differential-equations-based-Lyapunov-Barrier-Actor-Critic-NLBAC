...
class UnicycleEnv(gym.Env):
    ...
    def reset(self):
        ...
        low_safe_bound = np.array([-2.5, -2.5])
        high_safe_bound = np.array([2.5, 2.5])
        state_space = spaces.Box(low=low_safe_bound, high=high_safe_bound)
        self.state = state_space.sample()
        self.state = np.append(self.state, np.array([0.]))
        l_p = 0.03
        x0 = self.state[0] + l_p * np.cos(self.state[2])
        x1 = self.state[1] + l_p * np.sin(self.state[2])
        self.center = np.array([x0, x1])
        self.next_center = self.center
