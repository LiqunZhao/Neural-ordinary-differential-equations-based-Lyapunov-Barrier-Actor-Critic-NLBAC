...
class SAC_CBF_CLF(object):
    ...
    def __init__(self, num_inputs, action_space, env, args):
        ...
        # The NODE model. Remember to change the path when necessary
        neural_ode_model_pt = '/home/liqun/Neural-ordinary-differential-equations-based-Lyapunov-Barrier-Actor-Critic-NLBAC/Unicycle/Unicycle_RL_training/sac_cbf_clf/euler_horizon-100_singlehorizon-10_50_128.pt'
        self.neural_ode_model = NeuralODEModel(3, 3, 6)
        self.neural_ode_model.load_state_dict(torch.load(neural_ode_model_pt,map_location = self.device))
    ...
    def update_parameters(self, memory, batch_size, updates, dynamics_model, NODE_memory,NODE_model_update_interval):
        ...
        state_batch, action_batch, reward_batch, constraint_batch, center_pos_batch, next_center_pos_batch, next_state_batch, mask_batch, t_batch, next_t_batch = memory.sample(
        ...
        if updates % NODE_model_update_interval == 0:
            ...
            node_obs_batch, node_action_batch, node_reward_batch, node_constraint_batch, node_center_pos_batch, node_next_center_pos_batch, node_next_obs_batch, node_mask_batch, node_t_batch, node_next_t_batch = NODE_memory.sample(
        ...
        backup_policy_loss_2 = self.backup_get_cbf_clf_part(state_batch, backup_pi, dynamics_model,updates)
    ...
    def get_cbf_clf_part(self, obs_batch, action_batch, dynamics_model, center_pos_batch, updates):
        ...
        policy_loss_2 = self.get_policy_loss_2(state_batch, action_batch,lyapunov_value_detach,updates)

        return policy_loss_2

    def backup_get_cbf_clf_part(self, obs_batch, action_batch, dynamics_model,updates):
        ...
        state_batch = dynamics_model.get_state(obs_batch)          #convert obs to state
        backup_policy_loss_2 = self.backup_get_policy_loss_2(state_batch, action_batch,updates)

        return backup_policy_loss_2

    def get_policy_loss_2(self, state_batch, action_batch,lyapunov_value,updates):
        ...
        matr = torch.cat((cbf_term,lya_term),1)
    ...
    def backup_get_policy_loss_2(self, state_batch, action_batch,updates):
        ...
        # Update the coefficient of the augmented squared terms. Here we use the same coefficient used by the primary controller
        self.augmented_term = self.augmented_term * self.augmented_ratio
        self.augmented_term = min(self.augmented_term,200)            # Here set an upper bound for numerical stability

        backup_policy_loss_2 = float(self.backup_lambda_values[0]) * (backup_required_matrix[0] - self.cost_limit) + self.augmented_term / 2.0 * (backup_required_matrix[0] - self.cost_limit) * (backup_required_matrix[0] - self.cost_limit)
        for i in range(backup_required_matrix.shape[0] - 1):
            backup_policy_loss_2 += float(self.backup_lambda_values[i + 1]) * (backup_required_matrix[i + 1] - self.cost_limit) + self.augmented_term / 2.0 * (backup_required_matrix[i + 1] - self.cost_limit) * (backup_required_matrix[i + 1] - self.cost_limit)
