...
class NeuralODEModel(nn.Module):
    ...
    def forward(self, t, s):
        ...
        a_t = torch.zeros(s_action_time.shape).to(device)
...
def train_step(model, state, action, next_state,time_batch, optimizer, loss_func, horizon, solver, time_interval) -> float:
    """
    ...
    """
    assert len(state.shape) == 2 and len(action.shape) == 2 and len(next_state.shape) == 2 and len(time_batch.shape) == 2, print(
        state.shape,
        action.shape,
        next_state.shape,time_batch.shape)
    ...
    action_t = action
    time_t = time_batch
    action_time = torch.cat((action_t, time_t), dim=-1)
    expand_dim = len(state_t.shape) == 1
    if expand_dim:
        state_t = state_t.unsqueeze(0)
        action_t = action_t.unsqueeze(0)
    model_input_t = torch.cat((state_t, action_time), dim=-1)
    ...
    next_state_t = odeint(model, model_input_t, t_span, method=solver, atol=1e-7, rtol=1e-5)[
        -1]  # Predicted value
    next_state_pred = next_state_t[:, :10]
