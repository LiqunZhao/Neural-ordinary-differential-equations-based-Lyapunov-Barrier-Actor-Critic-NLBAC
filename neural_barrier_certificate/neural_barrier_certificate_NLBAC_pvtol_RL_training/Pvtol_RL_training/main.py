...
def train(agent, env, dynamics_model, args,logger_kwargs=dict()):
    ...
    for i_episode in range(args.max_episodes):
        ...
        while not done:
            ...
            next_obs, reward,constraint,barrier_signal,lya_pre_term,lya_next_term, done, info = env.step(action)     # Step
            ...
            memory.push(obs, action, reward, constraint, barrier_signal, lya_pre_term, lya_next_term, next_obs, mask,
                        t=episode_steps * env.dt,
                        next_t=(episode_steps + 1) * env.dt)

            # Append transition to NODE_memory
            NODE_memory.push(obs, action, reward, constraint,barrier_signal, lya_pre_term, lya_next_term, next_obs, mask,
                            t=episode_steps * env.dt,
                            next_t=(episode_steps + 1) * env.dt)
...
if __name__ == "__main__":
    ...
    writer = wandb.init(
        project='Node_neural_barrier_certificate_LBAC_Pvtol',  #   Name of the group where data are saved in wandb
        config=args,
        dir='wandb_logs',
        group='Pvtol',
    )
